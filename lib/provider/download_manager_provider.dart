...
class DownloadTask {
  ...
  /// Absolute folder to save this track into. When null, falls back to the
  /// global download location. Used by the "Download to folder" picker so a
  /// streamed track can land directly in a chosen local-library folder.
...
class DownloadManagerNotifier extends Notifier<List<DownloadTask>> {
  ...
  void addToQueue(SpotubeFullTrackObject track, {String? destinationDir}) {
  ...
  Future<void> _downloadTrack(DownloadTask task) async {
    try {
      ...
      // Per-task destination (from the folder picker) wins over the global
      // download location. Ensure it exists before writing into it.
      final saveDir = task.destinationDir ?? downloadLocation;
