...
abstract class ServiceUtils {
  ...
  /// Parse an album release date that may be a full ISO date ("2021-05-01"),
  /// a year-month ("2021-05"), or a bare year ("2021"). Returns the epoch for
  /// null/empty/unparseable values so they sort consistently to the bottom.
  static DateTime parseAlbumReleaseDate(String? raw) {
    if (raw == null || raw.isEmpty) {
      return DateTime.fromMillisecondsSinceEpoch(0);
    }
    final direct = DateTime.tryParse(raw);
    if (direct != null) return direct;
    // Fall back to year-only / year-month forms.
    final parts = raw.split('-');
    final year = int.tryParse(parts.first);
    if (year == null) return DateTime.fromMillisecondsSinceEpoch(0);
    ...
    return List<T>.from(tracks)
      ..sort((a, b) {
        switch (sortBy) {
          ...
          case SortBy.newest:
            // Newest release first. Local tracks carry releaseDate from the
            // ID3 year tag (localTrackFromFile); untagged files fall back to
            // the epoch and group together at the end.
            return parseAlbumReleaseDate(b.album.releaseDate)
                .compareTo(parseAlbumReleaseDate(a.album.releaseDate));
          case SortBy.oldest:
            return parseAlbumReleaseDate(a.album.releaseDate)
                .compareTo(parseAlbumReleaseDate(b.album.releaseDate));
